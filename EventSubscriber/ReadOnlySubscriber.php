...
class ReadOnlySubscriber implements EventSubscriber
{
    ...
    {
        $implements = class_implements($eventArgs->getClassName());
        if (is_array($implements)) {
            foreach ($implements as $implement) {
                if ($implement === ReadOnlyEntityInterface::class) {
                    $eventArgs->setFoundMetadata(
                        $eventArgs->getObjectManager()->getClassMetadata(
                            get_parent_class($eventArgs->getClassName())
                        )
                    );
